...
page_init(void)
{
	...
	page_free_list = NULL;
	for (i = 1; i < npages; ++i) {
		if (IOPHYSMEM <= PGSIZE * i && PGSIZE * i < PADDR(nextfree))
			continue;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	pte_t *pte;
	perm |= PTE_P;
	for (ptr = va; ptr < ROUNDUP(va + len, PGSIZE); ptr = ROUNDDOWN(ptr + PGSIZE, PGSIZE)) {
		if ((uint32_t)ptr < ULIM) {
