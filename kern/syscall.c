	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (dstva >= (void *)UTOP || PGOFF(dstva) != 0)
		return -E_INVAL;
	if (!(srcpp = page_lookup(srcenv->env_pgdir, srcva, &pte_store)))
		return -E_INVAL;
        if (perm & ~PTE_SYSCALL)
                return -E_INVAL;
        if (!((perm & PTE_U) && (perm & PTE_P)))
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void *)UTOP) {
		...
		if ((perm & (PTE_P|PTE_U)) != (PTE_P|PTE_U))
			return -E_INVAL;
		if (!(srcpp = page_lookup(curenv->env_pgdir, srcva, &pte_store)))
			return -E_INVAL;
		if ((perm & PTE_W) && !(*pte_store & PTE_W))
			return -E_INVAL;
		if (env->env_ipc_dstva && env->env_ipc_dstva < (void *)UTOP) {
			if (page_insert(env->env_pgdir, srcpp, env->env_ipc_dstva, perm) < 0)
				return -E_NO_MEM;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_env_set_priorty:
		return sys_env_set_priorty((envid_t)a1, (int)a2);
	case SYS_env_set_trapframe:
		return sys_env_set_trapframe((envid_t)a1, (struct Trapframe *)a2);
	case SYS_exec:
		return sys_exec((struct Elf*)a1, (uint32_t)a2);
	default:
		return -E_INVAL;
	}
}
